...
verdicts. If the header is absent, every field is "none"/"unknown" — never
guessed. A single failing mechanism never triggers a phishing verdict on its
own; that combination happens later in risk_engine.py.
"""
import re
...
def analyze_authentication(parsed: ParsedEmail) -> dict:
    ...
    spf = _first(_SPF_RE) or "unknown" if parsed.authentication_results else "none"
    dkim = _first(_DKIM_RE) or "unknown" if parsed.authentication_results else "none"
    dmarc = _first(_DMARC_RE) or "unknown" if parsed.authentication_results else "none"
    ...
    def _details(mechanism: str, status: str) -> str:
        if status == "none":
            return f"No {mechanism.upper()} result was present in Authentication-Results."
        if status == "unknown":
    ...
    return {
        ...
        "note": "Authentication results are one input among several; SPF/DKIM/DMARC failure alone does not imply phishing, "
                "and a DMARC pass alone does not imply legitimacy.",
    }


def auth_signals(auth: dict) -> list[dict]:
    """Each failing mechanism contributes a modest, explainable signal.
    Deliberately no single failure is enough to reach a phishing verdict —
    the risk engine combines this with content/URL/header evidence."""
    signals = []
    if auth["spf"] == "fail":
    ...
    if auth["spf"] == "none" and auth["dkim"] == "none" and auth["dmarc"] == "none":
        signals.append({"name": "no_authentication_data", "severity": "low", "weight": 3,
                         "explanation": "No Authentication-Results header was present, so SPF/DKIM/DMARC could not be evaluated."})
    return signals
