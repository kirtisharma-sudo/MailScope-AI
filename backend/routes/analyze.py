...
from services.risk_engine import fuse_signals, classify_from_score, confidence_from_signals
...
async def analyze(
    ...
    nlp_result = classify_content(parsed.body_text)
    ...
    risk_score, all_signals = fuse_signals(all_signals)
    classification = classify_from_score(risk_score, nlp_result.label, u_signals, auth)
    confidence = confidence_from_signals(len(all_signals), risk_score)
